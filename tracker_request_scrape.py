...
def scrape_udp(parsed_tracker, hashes):
	url = parsed_tracker.geturl()[3:]
	url = "http" + url
	hostname = urlparse(url).hostname
	port = urlparse(url).port
	print "Scraping UDP: %s " % (parsed_tracker.geturl())
	if len(hashes) > 74:
...
def scrape_http(parsed_tracker, hashes):
	print "Scraping HTTP: %s" % (parsed_tracker.geturl())
	qs = []
	for hash in hashes:
...
def udp_create_scrape_request(connection_id, hashes):
	...
	for hashx in hashes:		
		# urllib.unquote(hashx)
		# hex_repr = binascii.a2b_hex(urllib.unquote(hashx))
		buf += struct.pack("!20s", urllib.unquote(hashx))
	return (buf, transaction_id)
